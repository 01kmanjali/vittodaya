...
import { getActiveLoanProducts } from "@/constants/loans";
import { ArrowRight, Shield, Zap, Award, Headphones, TrendingUp, FileText, HelpCircle, User, Factory, Car, Home } from "lucide-react";

const topSchemes = fdSchemes
  .filter(s => s.featuredOrder)
  .sort((a, b) => (a.featuredOrder ?? 99) - (b.featuredOrder ?? 99))
  .slice(0, 3);

const loanProducts = getActiveLoanProducts();
              ...
              <div className="flex flex-wrap gap-4">
                <Button asChild size="lg" className="text-white shadow-lg animate-pulse-glow" style={{ background: "linear-gradient(135deg, var(--secondary) 0%, var(--secondary-light) 100%)" }}>
                  <Link href="/loans/personal">
                    Apply for a Loan <ArrowRight className="h-4 w-4 ml-1" />
                  </Link>
                </Button>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {loanProducts.map((loan, i) => {
              ...
              return (
                <Link key={loan.id} href={`/loans/${loan.type}`} className={`animate-fade-up stagger-${i + 1}`}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {topSchemes.map((scheme, i) => (
              <Card key={scheme.id} className={`hover-lift animate-fade-up stagger-${i + 1}`}>
                <CardContent className="p-5">
                  <div className="flex items-center gap-3 mb-4">
                    <Avatar className="w-11 h-11 rounded-xl" bankName={scheme.bankName} />
                    <div>
                  ...
                  <Button asChild className="w-full text-white" style={{ background: "linear-gradient(135deg, var(--secondary) 0%, var(--secondary-light) 100%)" }}>
                    <Link href={`/fd/${scheme.id}`}>Invest Now</Link>
                  </Button>
                </CardContent>
              </Card>
            ))}
      ...
      <section className="py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12 animate-fade-up">
            ...
            <p className="text-sm mt-2" style={{ color: "var(--text-secondary)" }}>Your trusted financial partner for growth and security</p>
          </div>
              ...
              <Link key={ql.href} href={ql.href} className={`animate-fade-up stagger-${i + 1}`}>
                <Card className="hover-lift cursor-pointer group">
                  <CardContent className="p-6 flex items-start gap-4">
                    <div className="w-11 h-11 rounded-xl flex items-center justify-center shrink-0 transition-colors" style={{ background: "#eff6ff" }}>
...
// Local Avatar fallback component used above
function Avatar({ bankName, className }: { bankName: string; className?: string }) {
  return (
    <div className={`flex items-center justify-center font-bold text-white text-sm rounded-xl ${className ?? ""}`} style={{ background: "var(--primary)" }}>
      {bankName.charAt(0)}
    </div>
  );
}
